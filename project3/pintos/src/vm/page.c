...
static unsigned hash_func_for_vm (const struct hash_elem *e, void *aux UNUSED) {
    struct vm_entry* vmE = hash_entry(e, struct vm_entry, hash_elem); /* Search vm_entry for hash element. */
...
static bool less_func_for_vm (const struct hash_elem *e1, const struct hash_elem *e2) {
    struct vm_entry* vmE1 = hash_entry(e1, struct vm_entry, hash_elem); /* Search vm_entry 1 for hash element. */
    struct vm_entry* vmE2 = hash_entry(e2, struct vm_entry, hash_elem); /* Search vm_entry 2 for hash element. */
    bool result = vmE1->vaddr < vmE2->vaddr; /* Compare them. */
    return result;
}
...
bool insert_vm_entry (struct hash *vm, struct vm_entry *vmE) {
    ...
    bool success;
    if (old == NULL) {
        success = true; /* Inserting vm_entry into hash table is done. */
    } else {
